...
local grammar = P {
	...
	-- CMTs probably shouldn't count as empty matches even if they match literally nothing, because this:
	-- either match the newline or match the beginning of the file
	filestart = Cmt(Cp(), function(_, _, mypos)
		return mypos == 1
	end),

	-- eof = P(-1), -- CHANGING THIS SHOULD NOT BREAK LIST HANDLING WTF
	eof = Cmt(Cp(), function(body, _, mypos)
		return mypos == (string.len(body) + 1)
	end),

	empty_line = V "newline" * S "\t " ^ 0 * #(V "newline" + V "eof"),
	-- empty line SHOULD go in here. so that nextline can do as many as it wants, it just ensures you get the next line
	-- but it can't because something is unsound
	...
	-- levels is in the nats and we are guaranteed to not increment more than 1 per indent
	-- you can't have a level that isn't a successor of a previous nat, so a stack is superfluous
